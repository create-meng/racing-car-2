...
class NavThroughPosesClient(Node):
    def __init__(self):
        ...
        self._goal_handle = None # 新增：用于记录当前任务句柄
    ...
    def get_result_callback(self, future):
        # 无论是第一阶段正常走完，还是被二维码中断取消，都会进入这里
        self.execute_next_phase()

    def execute_next_phase(self):
        # 如果还没收到二维码，说明是第一阶段dating.csv正常跑完了，继续等待
        if self.qr_result is None:
            self.get_logger().info("dating.csv 跑完，等待二维码识别...")
            for i in range(30):
