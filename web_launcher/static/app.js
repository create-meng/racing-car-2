...
function setConfigMessage(text, kind = "") {
  const message = document.querySelector("#nav-config-message");
  if (!message) return;
...
async function loadNavConfig() {
  const editor = document.querySelector("#nav-config-editor");
  setConfigMessage("正在读取导航参数...");
  const data = await requestJson("/api/nav_config");
  document.querySelector("#nav-config-path").textContent = data.path;
  editor.value = data.text || "";
  setConfigMessage("已加载小车上的导航参数", "success");
}

async function saveNavConfig() {
  const editor = document.querySelector("#nav-config-editor");
  setConfigMessage("正在保存导航参数...");
  const data = await requestJson("/api/nav_config", {
    method: "POST",
...
async function handleClick(event) {
  ...
  try {
    ...
    if (action === "load-nav-config") {
      await loadNavConfig();
    }
    if (action === "save-nav-config") {
      await saveNavConfig();
    }
    await refreshStatus();
  } catch (error) {
    if (action === "load-nav-config" || action === "save-nav-config") {
...
refreshStatus();
loadNavConfig().catch((error) => setConfigMessage(error.message, "error"));
